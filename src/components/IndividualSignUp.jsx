...
//main code before distribution limit logic 
...
import { Link, useNavigate } from 'react-router-dom';
import toast from 'react-hot-toast';
import { FaIdCard, FaUser, FaEnvelope, FaPhone, FaLock, FaGlobe, FaMapMarkerAlt, FaArrowRight, FaChevronDown, FaUserTie, FaCheckCircle, FaMinusCircle, FaPlusCircle, FaVenusMars, FaCalendarAlt, FaEye, FaEyeSlash, FaGift, FaExclamationTriangle } from 'react-icons/fa';
import { IoFlagSharp } from 'react-icons/io5';
import Login from './Login';
import Navbar from './Navbar';
...
const IndividualSignUp = () => {
  ...
  const onSubmit = async (data) => {
    ...
    if (data.password !== data.confirmpassword) {
      toast.error("Passwords do not match! 🔒", { duration: 5000, icon: <FaExclamationTriangle className="text-red-500" /> });
      return;
    }
    ...
    try {
      ...
      if (message.toLowerCase().includes("successfully")) {
        localStorage.setItem("individualSignUpData", JSON.stringify(payload));
        toast.success("Individual Signup Successful! 🎉", { duration: 4000 });
        localStorage.removeItem("verifiedEmail");
        navigate("/");
      } else {
        // Handle specific promo code errors with clear alerts
        if (message.includes("Invalid promo code")) {
          toast.error("Invalid promo code! ❌ Please enter a valid promo code.", { 
            duration: 5000,
            icon: <FaExclamationTriangle className="text-red-500" />
          });
        } else if (message.includes("Promo code is not valid or has expired")) {
          toast.error("Promo code has expired! ⏰ Please use an active promo code.", { 
            duration: 5000,
            icon: <FaExclamationTriangle className="text-red-500" />
          });
          ...
          toast.error(message || "Signup failed! 😔 Please check your details and try again.", { 
            duration: 5000,
            icon: <FaExclamationTriangle className="text-red-500" />
          });
        }
      ...
      toast.error("Failed to sign up due to a network error. ⚠️ Please try again later.", { 
        duration: 5000,
        icon: <FaExclamationTriangle className="text-red-500" />
      });
    }
        ...
        <div className="w-full max-w-md bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 transition-all duration-300 hover:shadow-2xl">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Promo Code (Optional) <FaGift className="inline ml-1 text-sky-500" />
                </label>
                <div className="relative group">
                  <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                    <FaGift className="text-gray-400 group-hover:text-sky-500 transition-colors" />

...
const ComBubChart = () => {
  ...
  useEffect(() => {
    const fetchGraphData = async () => {
      ...
      const cacheKey = `combined_bubble_chart_${uploadId}`;
      ...
      try {
        const response = await axios.post(
          `${API_BASE}/file/generate_combined_bubble_chart`,
          new URLSearchParams({ uploadId })
        );

        console.log("ComBubChart Data:", response.data);
        ...
        const convertedData = response.data.figure.data.map((trace) => ({
          ...
          x: Array.isArray(trace.x)
            ? trace.x.map((value) => (!isNaN(value) ? parseFloat(value) : value))
            : trace.x,
        ...
        setError("Graph generation failed. Please check the data or try again.");
        console.error("Graph ComBubChart API Error:", err.response ? err.response.data : err.message);
      }
    };
